...
import style from './jobs/resumeHeading.module.css'
...
export const JobsList = () => {
    ...
    function RemoveItem(index) {
        let arrayData = userDataProvider.jobs;
        arrayData.splice(index, 1);
        setUserData({...userDataProvider, jobs: [...arrayData]})
        console.log(arrayData)
    }

    return (<>
        <div className={style.max_width}>
            <div className={style.heading}>Job Summary</div>
            <div className={style.educationContainer}>
                {
                    userDataProvider.jobs.map((value, index) => <div className={style.educationLi} key={index}>
                             <span className={style.paraTag}>
                            {index + 1}
                        </span>
                            <div className={style.ed_degree}>
                                {value.jobTitle}
                            </div>
                            <span className={style.ed_schoolName}>
                           | {value.companyName} |
                        </span>
                            <span className={style.ed_period}>
            ...
            <input type="file" name="" id="" onChange={(e) => setFile({
                'preview': URL.createObjectURL(e.target.files[0]),
                'raw': e.target.files[0]
            })}/>
            <div className={style.addDegree} onClick={() => navigate('/addJob')}>
                ADD Another job
            </div>
            <div className={`${style.buttonContainer}${style.educationbuttons}`}>
                <button type={'button'} className={style.back}>
                    BACK
                </button>
                <button type={'submit'} onClick={submit} className={style.next}>Post</button>
            </div>
